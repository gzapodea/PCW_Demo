...
ITPA_USER = "Jim"  # Change this to match your user
ITPA_EMAIL = "your_email_address"


def main():
    """
    This IT Personal Assistant App will:
    - the application will be triggered by a CMX notification, Systems Manager, or Meraki Location API
    - when user arrives to the office it will create a Spark Room and welcome the user to the office
    - it will check for any open ServiceNow incidents from the caller {SNOW_DEV} (the {IOSXE} account)
    - post the summary of the urgent incidents in Spark
    """
